...
function summariseReletOpportunities(): string {
  ...
  return [
    ...
    "### Conversion & margin",
    "",
    `• **Cover conversion (take-up)** is **${cover?.value ?? "14%"}** — share of bookings buying Flexible Cancellation.`,
    ...
    "",
    `• **Guests cancelled** at **${cancel?.value ?? "10%"}** — some cancel is normal when guests have cover; the commercial question is recovery.`,
    `• **Re-let** is **${relet?.value ?? formatPct(snap.reletRate)}**` +
      (marketRelet
        ? ` (market **${marketRelet.marketLabel}**, ${marketRelet.trend}).`
        : "."),
    `• **Extra revenue** from re-lets sits at **${extra?.value ?? "£100k"}** — proof the loop pays.`,
    "",
...
function summariseBookingTypeDrilldown(bedroomLabel: string, departureLabel: string): string {
  ...
  return [
    ...
    `• **Guests cancelled:** ${overall.cancel}% (${gapLabel(portfolioGap.cancel)})`,
